...
contract ClaimsManager is InitializableV2 {
    ...
    ERC20Mintable private colivingToken;
    ...
    ) public initializer
    {
        ...
        colivingToken = ERC20Mintable(_tokenAddress);
    ...
    function initiateRound() external {
        ...
        if (recurringCommunityFundingAmount > 0 && communityPoolAddress != address(0x0)) {
            ...
            colivingToken.mint(address(this), recurringCommunityFundingAmount);

            // Approve transfer to community pool address
            colivingToken.approve(communityPoolAddress, recurringCommunityFundingAmount);

            // Transfer to community pool address
            ERC20(address(colivingToken)).safeTransfer(communityPoolAddress, recurringCommunityFundingAmount);
    ...
    ) external returns (uint256)
    {
        ...
        // ERC20Mintable always returns true
        colivingToken.mint(address(this), rewardsForClaimer);

        // Approve transfer to staking address for claimer rewards
        // ERC20 always returns true
        colivingToken.approve(stakingAddress, rewardsForClaimer);
