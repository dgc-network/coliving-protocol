...
contract EthRewardsManager is InitializableV2 {
    ...
    function initialize(
        ...
        colivingToken = ERC20(_tokenAddress);
    ...
    function transferToSolana(uint256 arbiterFee, uint32 _nonce) external {
        ...
        uint256 balance = colivingToken.balanceOf(address(this));
        colivingToken.approve(address(wormhole), balance);

        wormhole.transferTokens(
            address(colivingToken),
            balance,
            1,
            recipient,
            arbiterFee,
