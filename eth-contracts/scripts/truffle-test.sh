...
if docker ps | grep 'coliving_ganache_cli_eth_contracts_test' >/dev/null; then
	# killing the container seems to be faster than restarting
	printf 'Remove old containers and build artifacts\n'
	docker rm -f coliving_ganache_cli_eth_contracts_test
	rm -rf ./build/
	printf '\n'
fi
...
docker run --name coliving_ganache_cli_eth_contracts_test -d -p 8556:8545 trufflesuite/ganache-cli:latest -h 0.0.0.0 -l 8000000 -a 500 -k istanbul
...
elif [ $1 == '--coliving-random' ] && [ $# -eq 1 ]; then
