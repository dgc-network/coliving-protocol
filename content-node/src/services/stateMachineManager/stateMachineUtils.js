    ...
    replicas.map(async (replica) => {
      ...
      for (
        ...
        batchClockStatusResp.forEach((clockStatusResp) => {
          /**
           * @notice `filesHash` will be null if node has no files for user. This can happen even if clock > 0 if user has ColivingUser or DigitalContent table records without any File table records
           */
