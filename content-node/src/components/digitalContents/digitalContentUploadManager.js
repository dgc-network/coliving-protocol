...
class DigitalContentUploadManager {
  ...
  static async transcodeAndSegment({ logContext }, { fileName, fileDir }) {
    ...
    try {
      ...
      FileManager.removeDigitalContentFolder({ logContext }, fileDir)
   ...
   *  returns
   *  {
   *    trancodedDigitalContentCID {string} : the transcoded digital_content's CID representation,
   *    transcodedDigitalContentUUID {string} : the transcoded digital_content's UUID in the Files table,
  ...
  static async processTranscodeAndSegments(
    ...
    const digitalContentSegments = createSegmentToDurationMap({
      segmentFileResult,
      segmentDurations,
      logContext,
      fileDir
    ...
    return {
      transcodedDigitalContentCID: transcodeFileResult.multihash,
      transcodedDigitalContentUUID: transcodeFileUUID,
...
function createSegmentToDurationMap({
  ...
  let digitalContentSegments = segmentFileResult.map((segmentFile) => {
    return {
  ...
  digitalContentSegments = digitalContentSegments.filter((digitalContentSegment) => digitalContentSegment.duration)

  // error if there are no digital_content segments
  if (!digitalContentSegments || !digitalContentSegments.length) {
    // Prune upload artifacts
    FileManager.removeDigitalContentFolder({ logContext }, fileDir)
