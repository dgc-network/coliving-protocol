...
const { removeDigitalContentFolder } = require('../../fileManager')
...
class DigitalContentTranscodeHandoffManager {
  static logContext = {}
  static logger = genericLogger.child(DigitalContentTranscodeHandoffManager.logContext)
   ...
   * }
   *
   * See routes/digitalContents.js for more information
   * @returns
   */
  static async handOff({ logContext }, req) {
    const logger = DigitalContentTranscodeHandoffManager.initLogger(logContext)
    ...
    try {
      ...
      sps = await DigitalContentTranscodeHandoffManager.selectRandomSPs(libs)
    } catch (e) {
    ...
    for (const sp of sps) {
      ...
      try {
        ...
        const transcodeAndSegmentUUID =
          await DigitalContentTranscodeHandoffManager.sendDigitalContentToSp({ sp, req })

        decisionTree.state = HAND_OFF_STATES.POLLING_FOR_TRANSCODE
        const polledTranscodeResponse =
          await DigitalContentTranscodeHandoffManager.pollForTranscode({
            sp,
        ...
        const localFilePaths =
          await DigitalContentTranscodeHandoffManager.fetchFilesAndWriteToFs({
   ...
   */
  static async sendDigitalContentToSp({ sp, req }) {
    const logger = DigitalContentTranscodeHandoffManager.logger
    const { fileDir, fileName, fileNameNoExtension } = req

    await DigitalContentTranscodeHandoffManager.fetchHealthCheck(sp)

    logger.info({ sp }, `Sending off transcode and segmenting request...`)
    const transcodeAndSegmentUUID =
      await DigitalContentTranscodeHandoffManager.sendTranscodeAndSegmentRequest({
        sp,
        fileDir,
        fileName,
        fileNameNoExtension
  ...
  static async pollForTranscode({ uuid, sp }) {
    ...
    return asyncRetry({
      logger: DigitalContentTranscodeHandoffManager.logger,
      asyncFn: async (bail, num) => {
        ...
        const { status, resp } =
          await DigitalContentTranscodeHandoffManager.fetchTranscodeProcessingStatus({
            sp,
            uuid
          })
  ...
  static async fetchFilesAndWriteToFs({
    ...
    try {
      ...
      for (let i = 0; i < segmentFileNames.length; i++) {
        ...
        res = await DigitalContentTranscodeHandoffManager.fetchSegment(
          sp,
          segmentFileName,
          fileNameNoExtension
        )
      ...
      res = await DigitalContentTranscodeHandoffManager.fetchTranscode(
        sp,
        fileNameNoExtension
      )
      ...
      res = await DigitalContentTranscodeHandoffManager.fetchM3U8File(
        sp,
        fileNameNoExtension
      )
      ...
      removeDigitalContentFolder(DigitalContentTranscodeHandoffManager.logContext, fileDir)
      throw e
    }
  ...
  static async sendTranscodeAndSegmentRequest({
    ...
    const originalDigitalContentFormData =
      await DigitalContentTranscodeHandoffManager.createFormData(
        fileDir + '/' + fileName
      )
    ...
    const resp = await DigitalContentTranscodeHandoffManager.asyncRetryNotOn404({
      logger: DigitalContentTranscodeHandoffManager.logger,
      asyncFn: async () => {
        return axios({
          url: `${sp}/transcode_and_segment`,
          method: 'post',
          data: originalDigitalContentFormData,
          headers: {
            ...originalDigitalContentFormData.getHeaders()
          },
          params: {
  ...
  static async fetchTranscodeProcessingStatus({ sp, uuid }) {
    ...
    const { data: body } =
      await DigitalContentTranscodeHandoffManager.asyncRetryNotOn404({
        logger: DigitalContentTranscodeHandoffManager.logger,
        asyncFn: async () => {
          return axios({
  ...
  static async fetchSegment(sp, segmentFileName, fileNameNoExtension) {
    ...
    return DigitalContentTranscodeHandoffManager.asyncRetryNotOn404({
      logger: DigitalContentTranscodeHandoffManager.logger,
      asyncFn: async () => {
        return axios({
          url: `${sp}/transcode_and_segment`,
  ...
  static initLogger(logContext) {
    DigitalContentTranscodeHandoffManager.logContext = logContext
    DigitalContentTranscodeHandoffManager.logger = genericLogger.child(logContext)

    return DigitalContentTranscodeHandoffManager.logger
  }
...
module.exports = DigitalContentTranscodeHandoffManager
