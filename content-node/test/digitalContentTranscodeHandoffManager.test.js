...
const DigitalContentTranscodeHandoffManager = require('../src/components/digital_contents/DigitalContentTranscodeHandoffManager')

describe('test DigitalContentTranscodeHandoffManager', function () {
  let libsMock, reqMock
  beforeEach(async () => {
    libsMock = getLibsMock()
    reqMock = {
      logContext: { env: 'DigitalContentTranscodeHandoffManager test' },
      fileDir: 'testFileDir',
  ...
  it('If selecting random sps fails, return empty response', async function () {
    sinon
      .stub(DigitalContentTranscodeHandoffManager, 'selectRandomSPs')
      .rejects(new Error('i have failed'))

    const resp = await DigitalContentTranscodeHandoffManager.handOff(
  ...
  it('When polling for transcode, if polling fails, return empty response', async function () {
    ...
    sinon
      .stub(DigitalContentTranscodeHandoffManager, 'sendDigitalContentToSp')
      .resolves(expectedUuid)

    sinon
      .stub(DigitalContentTranscodeHandoffManager, 'pollForTranscode')
  ...
  it('When libs call fails to get service provider list, throw error', async function () {
    ...
    try {
      await DigitalContentTranscodeHandoffManager.selectRandomSPs(libsMock)
      assert.fail(
  ...
  it('Selecting random SPs for transcode handoff works as expected', async function () {
    ...
    const randomSPs = await DigitalContentTranscodeHandoffManager.selectRandomSPs(
      libsMock
    )
    ...
    const oneSP = await DigitalContentTranscodeHandoffManager.selectRandomSPs(libsMock)
    assert.strictEqual(oneSP.length, 1)
    assert.ok(allSPsSet.has(oneSP[0]))
  })

  // sendDigitalContentToSp()

  it('When handing off transcode request to an sp, if the health check fails, throw error', async function () {
    sinon
      .stub(DigitalContentTranscodeHandoffManager, 'fetchHealthCheck')
      .rejects(new Error('failed to fetch health check response'))

    let resp
    try {
      resp = await DigitalContentTranscodeHandoffManager.sendDigitalContentToSp({
        sp: 'http://some_cn.com',
        req: reqMock
      })
  ...
  it('When handing off transcode request to an sp, if the health check passes but transcode req does not, throw error', async function () {
    sinon.stub(DigitalContentTranscodeHandoffManager, 'fetchHealthCheck').resolves()

    sinon
      .stub(DigitalContentTranscodeHandoffManager, 'sendTranscodeAndSegmentRequest')
  ...
  it('When handing off transcode request to an sp, if both the health check and transcode passes, return uuid', async function () {
    ...
    try {
      const actualUuid = await DigitalContentTranscodeHandoffManager.sendDigitalContentToSp({
        sp: 'http://some_cn.com',
        req: reqMock
      })
  ...
  it('When fetching segment and writing to fs, if fetching fails, throw error and no files from this call are written to tmp dir', async function () {
    sinon
      .stub(DigitalContentTranscodeHandoffManager, 'fetchSegment')
  ...
  it('When fetching transcode and writing to fs, if fetching fails, throw error and no files from this call are written to tmp dir', async function () {
    ...
    sinon
      .stub(DigitalContentTranscodeHandoffManager, 'fetchTranscode')
  ...
  it('When fetching m3u8 and writing to fs, if fetching fails, throw error and no files from this call are written to tmp dir', async function () {
    ...
    sinon
      .stub(DigitalContentTranscodeHandoffManager, 'fetchM3U8File')
  ...
  it('When fetching files and writing to fs, if fetching succeeds, return the file paths', async function () {
    ...
    try {
      const resp = await DigitalContentTranscodeHandoffManager.fetchFilesAndWriteToFs({
        fileNameNoExtension: reqMock.fileNameNoExtension,
        transcodeFilePath: `/test_file_storage/files/tmp_digital_content_artifacts/${reqMock.uuid}/${reqMock.uuid}.mp3`,
        segmentFileNames: ['segment00000.ts'],
        segmentFilePaths: [
