...
// it uses colivingLibs to add known service providers to prometheus.yml
...
const generateEnv = async (stream, env) => {
  ...
  const discoveryNodes = await colivingLibs.ethContracts.ServiceProviderFactoryClient.getServiceProviderList('discovery-node')
  const contentNodes = await colivingLibs.ethContracts.ServiceProviderFactoryClient.getServiceProviderList('content-node')
