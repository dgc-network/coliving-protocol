...
The information stored includes Coliving user metadata, images, and digitalcoin content. The content is backed by a local directory.
...
The indexed content includes user, digital_content, and album/contentList information along with social features. The data is stored for quick access, updated on a regular interval, and made available for clients via a RESTful API.
...
4.\) Set load balancer timeouts. Minimum timeouts are 1 hour \(3600 seconds\) for Creator Node requests and 1 minutes \(60 seconds\) for Discovery Node requests. DigitalContent uploads especially for larger files can take several minutes to complete.
