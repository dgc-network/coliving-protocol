...
Únete al canal de discord de operadores de nodo en el [servidor de discord de digitalcoin](https://discord.com/invite/)

## 0. Clonar el repositorio digitalcoin-k8 manifests
...
La información almacenada incluye metadatos de usuario de Coliving, imágenes y contenido de digitalcoin. El contenido está respaldado por un directorio local.
...
**Nota:** si aún no has registrado el servicio, introduce la url que planeas registrar en `contentNodeEndpoint`.
...
```text
kubectl aplicar -f digitalcoin/logger/logger.yaml
```
