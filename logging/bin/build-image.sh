...
docker build \
    -t coliving/filebeat:${FILEBEAT_VERSION} \
...
docker build \
    -t coliving/metricbeat:${METRICBEAT_VERSION} \
    --build-arg git_sha=$(git rev-parse HEAD) \
    --build-arg ELASTIC_ENDPOINT=${ELASTIC_ENDPOINT} \
    --build-arg ELASTIC_CLOUD_ID=${ELASTIC_CLOUD_ID} \
    --build-arg API_ID=${API_ID} \
...
if [[ "${1}" == "push" ]]; then
    docker push coliving/metricbeat:${METRICBEAT_VERSION}
    docker push coliving/filebeat:${FILEBEAT_VERSION}
fi
