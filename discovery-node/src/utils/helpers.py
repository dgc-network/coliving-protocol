...
fqdn_regex = re.compile(
    r"^(?:^|[ \t])((https?:\/\/)?(?:localhost|(cn[0-9]_content-node_1:[0-9]+)|[\w-]+(?:\.[\w-]+)+)(:\d+)?(\/\S*)?)$"
)
...
log_format = {
    ...
    "funcName": "funcName",
    "lineno": "lineno",
    "service": os.getenv("coliving_service", "default"),
}
...
def create_digital_content_route_id(title, handle):
    """
    Constructs a digital_content's route_id from an unsanitized title and handle.
...
def create_digital_content_slug(title, digital_content_id, collision_id=0):
    """Converts the title of a digital_content into a URL-friendly 'slug'
    ...
    hashed digital_content_id.

    Example:
    (Title="My Awesome DigitalContent!", collision_id=2) => "my-awesome-digital-content-2"
    """
    sanitized_title = title.encode("utf-8", "ignore").decode("utf-8", "ignore")
    # Strip out invalid character
    sanitized_title = re.sub(
    ...
    if not sanitized_title:
        sanitized_title = encode_int_id(digital_content_id)
