...
from .models.agreements import stem_full, digital_content, digital_content_full
...
ns = Namespace("agreements", description="DigitalContent related operations")
full_ns = Namespace("agreements", description="Full digital_content operations")
...
def get_single_digital_content(digital_content_id, current_user_id, endpoint_ns):
    ...
    single_digital_content = extend_digital_content(agreements[0])
...
class BulkAgreements(Resource):
    ...
    def get(self):
        ...
        if ids:
            agreements = get_digital_contents({"with_users": True, "id": decode_ids_array(ids)})
        else:
            agreements = get_digital_contents({"with_users": True, "routes": routes_parsed})
        if not agreements:
            if handle and slug:
        ...
        if handle and slug:
            agreements = extend_digital_content(agreements[0])
        else:
            agreements = [extend_digital_content(digital_content) for digital_content in agreements]
...
class FullBulkAgreements(Resource):
    ...
    def get(self):
        ...
        if slug and handle:
            routes_parsed.append({"handle": handle, "slug": slug})
        if ids:
            agreements = get_digital_contents(
                {
                    "with_users": True,
...
class AgreementStream(Resource):
    ...
    def get(self, digital_content_id):
        ...
        agreements = get_digital_contents(args)
        digital_content = agreements[0]
        if digital_content["is_delete"] or digital_content["user"]["is_deactivated"]:
            abort_not_found(digital_content_id, ns)

        primary_node = content_nodes[0]
        stream_url = urljoin(primary_node, f"agreements/stream/{digital_content_id}")
...
class AgreementSearchResult(Resource):
    ...
    @ns.doc(
        ...
        description="""Search for a digital_content or agreements""",
        responses={200: "Success", 400: "Bad request", 500: "Server error"},
    )
...
# `get_trending_digital_contents.py`, which caches the scored agreements before they are populated (keyed by genre + time).
...
@full_ns.route(
    ...
    doc={
        "get": {
            ...
            "description": """Gets the digital_content IDs of the top trending agreements on Coliving""",
            "responses": {200: "Success", 400: "Bad request", 500: "Server error"},
        }
    },
)
@full_ns.route(
    ...
    doc={
        "get": {
            ...
            "description": """Gets the digital_content IDs of the top trending agreements on Coliving based on the given trending strategy version""",
            "params": {"version": "The strategy version of trending to use"},
            "responses": {200: "Success", 400: "Bad request", 500: "Server error"},
        }
    },
...
class FullRemixableAgreements(Resource):
    ...
    def get(self):
        ...
        agreements = get_remixable_digital_contents(args)
...
class FullRemixesRoute(Resource):
    ...
    def get(self, digital_content_id):
        ...
        response["agreements"] = list(map(extend_digital_content, response["agreements"]))
...
class FullRemixingRoute(Resource):
    ...
    @full_ns.doc(
        ...
        description="""Gets all the agreements that the given digital_content remixes""",
        params={"digital_content_id": "A DigitalContent ID"},
    )
    @full_ns.expect(pagination_with_current_user_parser)
...
class BestNewReleases(Resource):
    ...
    def get(self):
        ...
        agreements = get_top_followee_windowed("digital_content", window, args)
        agreements = list(map(extend_digital_content, agreements))
        return success_response(agreements)
...
class MostLoved(Resource):
    ...
    def get(self):
        ...
        agreements = get_top_followee_saves("digital_content", args)
        agreements = list(map(extend_digital_content, agreements))
        return success_response(agreements)
