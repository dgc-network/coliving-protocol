...
from src.queries.get_trending_agreements import TRENDING_TTL_SEC
...
def get_time_trending(cache_args, time, limit, strategy):
    ...
    time_trending_agreement_ids = [
        {"agreement_id": agreement["agreement_id"]} for agreement in time_trending
    ]
    time_trending_agreement_ids = time_trending_agreement_ids[:limit]
    return time_trending_agreement_ids
...
def get_trending_ids(args, strategy):
    ...
    Args:
        ...
        args.limit: (number) The number of agreement ids to return
    ...
    Returns:
        trending_times_id: (dict) Dictionary containing the week/month/year trending agreement ids
    """
    ...
    week_trending_agreement_ids = get_time_trending(cache_args, "week", limit, strategy)
    month_trending_agreement_ids = get_time_trending(cache_args, "month", limit, strategy)
    year_trending_agreement_ids = get_time_trending(cache_args, "year", limit, strategy)

    return {
        "week": week_trending_agreement_ids,
        "month": month_trending_agreement_ids,
        "year": year_trending_agreement_ids,
    }
