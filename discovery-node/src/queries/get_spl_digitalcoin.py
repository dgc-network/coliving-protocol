...
from src.tasks.index_spl_token import cache_latest_spl_digitalcoin_db_tx
...
def get_latest_spl_digitalcoin() -> Optional[Dict]:
...
def get_latest_cached_spl_digitalcoin_db(redis: Redis) -> Optional[CachedProgramTxInfo]:
    latest_spl_digitalcoin_db = get_cache_latest_sol_program_tx(
        redis, latest_sol_spl_token_db_key
    )
    if not latest_spl_digitalcoin_db:
        # If nothing found in cache, pull from db
        token_tx = get_latest_spl_digitalcoin()
        if token_tx:
            latest_spl_digitalcoin_db = {
            ...
            if latest_spl_digitalcoin_db:
                cache_latest_spl_digitalcoin_db_tx(redis, latest_spl_digitalcoin_db)

    return latest_spl_digitalcoin_db


def get_latest_cached_spl_digitalcoin_program_tx(redis) -> CachedProgramTxInfo:
    # Latest spl digitalcoin tx from chain
    latest_spl_digitalcoin_program_tx = get_latest_sol_db_tx(
        redis, latest_sol_spl_token_program_tx_key
    )
    return latest_spl_digitalcoin_program_tx


def get_spl_digitalcoin_health_info(redis: Redis, current_time_utc: datetime):
    db_cache = get_latest_cached_spl_digitalcoin_db(redis)
    tx_cache = get_latest_cached_spl_digitalcoin_program_tx(redis)
