...
# $coliving_prometheus_container
coliving_prometheus_container = getenv("coliving_prometheus_container", "worker")

# remove all *container*.db files since we're restarting the process
files = glob(f"/{getenv('PROMETHEUS_MULTIPROC_DIR')}/*{coliving_prometheus_container}*")
for f in files:
...
def process_identifier():
    return f"{coliving_prometheus_container}_{getpid()}"
