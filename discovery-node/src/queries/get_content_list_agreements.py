...
def get_content_list_digital_contents(session, args):
    """Accepts args:
    {
        ...
        "current_user_id": int
        "populate_digital_contents": boolean # whether to add users & metadata to agreements
    }
    ...
    try:
        ...
        agreements = helpers.query_result_to_list(content_list_digital_contents)

        if args.get("populate_digital_contents"):
            current_user_id = args.get("current_user_id")
            agreements = populate_digital_content_metadata(
                session, list(digital_content_ids_set), agreements, current_user_id
            )

            add_users_to_digital_contents(session, agreements, current_user_id)

        # { digital_content_id => digital_content }
        digital_content_ids_map = {digital_content["digital_content_id"]: digital_content for digital_content in agreements}
