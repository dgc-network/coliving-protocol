...
def digital_content_state_update(
    ...
    pending_digital_content_routes: List[DigitalContentRoute] = []
    ...
    logger.info(
        f"index.py | digitalContents.py | [digital_content indexing] There are {num_total_changes} events processed and {skipped_tx_count} skipped transactions."
    )

    for digital_content_id, value_obj in digital_content_events.items():
        if value_obj["events"]:
            logger.info(f"index.py | digitalContents.py | Adding {value_obj['digital_content']}")
    ...
    if num_total_changes:
        ...
        logger.info(
            f"index.py | digitalContents.py | digital_content_state_update | finished digital_content_state_update in {datetime.now() - begin_digital_content_state_update} // per event: {(datetime.now() - begin_digital_content_state_update) / num_total_changes} secs"
        )
    return num_total_changes, digital_content_ids
...
def update_remixes_table(session, digital_content_record, digital_content_metadata):
    ...
    if "remix_of" in digital_content_metadata and isinstance(digital_content_metadata["remix_of"], dict):
        digitalContents = digital_content_metadata["remix_of"].get("digitalContents")
        if digitalContents and isinstance(digitalContents, list):
...
def update_digital_content_routes_table(
    ...
    if prev_digital_content_route_record is None:
        prev_digital_content_route_record = (
            ...
            .filter(
                DigitalContentRoute.digital_content_id == digital_content_record.digital_content_id,
                DigitalContentRoute.is_current == True,
            )  # noqa: E712
            .one_or_none()
        )
    ...
    if max_collision_id is None:
        max_collision_id = (
            session.query(functions.max(DigitalContentRoute.collision_id))
            .filter(
                DigitalContentRoute.title_slug == new_digital_content_slug_title,
                DigitalContentRoute.owner_id == digital_content_record.owner_id,
            )
            .one_or_none()
        )[0]

    existing_digital_content_route: Optional[DigitalContentRoute] = None
    ...
    if new_digital_content_slug[-1].isdigit():
        ...
        if existing_digital_content_route is None:
            existing_digital_content_route = (
                session.query(DigitalContentRoute)
                .filter(
                    DigitalContentRoute.slug == new_digital_content_slug,
                    DigitalContentRoute.owner_id == digital_content_record.owner_id,
                )
                .one_or_none()
            )
    ...
    new_digital_content_route = DigitalContentRoute()
...
def parse_digital_content_event(
    ...
    if event_type == digital_content_event_types_lookup["new_digital_content"]:
        ...
        if digital_content_record.cover_art:
            logger.info(
                f"index.py | digitalContents.py | Processing digital_content cover art {digital_content_record.cover_art}"
            )
    ...
    if event_type == digital_content_event_types_lookup["delete_digital_content"]:
        ...
        logger.info(f"index.py | digitalContents.py | Removing digital_content : {digital_content_record.digital_content_id}")
