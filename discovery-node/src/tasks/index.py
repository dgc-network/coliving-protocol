...
def revert_blocks(self, db, revert_blocks_list):
    ...
    with db.scoped_session() as session:
        ...
        for revert_block in revert_blocks_list:
            ...
            for content_list_to_revert in revert_content_list_entries:
                content_list_id = content_list_to_revert.content_list_id
                previous_content_list_entry = (
                    session.query(ContentList)
                    .filter(ContentList.content_list_id == content_list_id)
                    .filter(ContentList.blocknumber < revert_block_number)
                    .order_by(ContentList.blocknumber.desc())
                    .first()
                )
                if previous_content_list_entry:
                    previous_content_list_entry.is_current = True
...
def update_task(self):
    ...
    content_list_abi = update_task.abi_values[CONTENT_LIST_FACTORY_CONTRACT_NAME]["abi"]
    content_list_contract = update_task.web3.eth.contract(
        address=get_contract_addresses()[CONTENT_LIST_FACTORY], abi=content_list_abi
    )
