...
def update_digital_contents_is_available_status(db: Any, redis: Any) -> None:
    """Check digital_content availability on all unavailable agreements and update in Agreements table"""
    ...
    for i in range(0, len(all_unavailable_digital_content_ids), BATCH_SIZE):
        ...
        try:
            with db.scoped_session() as session:
                ...
                agreements = query_digital_contents_by_digital_content_ids(session, unavailable_digital_content_ids_batch)
...
def fetch_unavailable_digital_content_ids(node: str) -> List[int]:
    ...
    try:
        ...
        logger.warn(
            f"update_digital_content_is_available.py | Could not fetch unavailable agreements from {node}: {e}"
        )
