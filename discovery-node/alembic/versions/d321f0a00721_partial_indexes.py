...
def upgrade():
    ...
    CREATE INDEX IF NOT EXISTS digital_content_owner_id_idx ON digitalContents (owner_id) WHERE is_current;

    -- add index for digital_content_created_at
    CREATE INDEX IF NOT EXISTS digital_content_created_at_idx ON digitalContents (created_at) WHERE is_current;
