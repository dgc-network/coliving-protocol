...
CREATE MATERIALIZED VIEW IF NOT EXISTS public.contentList_lexeme_dict AS
 SELECT row_number() OVER (PARTITION BY true::boolean) AS row_number,
    words.contentList_id,
    words.contentList_name,
    words.owner_id,
    words.handle,
    words.user_name,
    words.repost_count,
    words.word
   FROM ( SELECT p.contentList_id,
            lower((p.contentList_name)::text) AS contentList_name,
            p.contentList_owner_id AS owner_id,
            lower((u.handle)::text) AS handle,
            lower(u.name) AS user_name,
            a.repost_count,
            unnest((tsvector_to_array(to_tsvector('public.coliving_ts_config'::regconfig, replace((COALESCE(p.contentList_name, ''::character varying))::text, '&'::text, 'and'::text))) || lower((COALESCE(p.contentList_name, ''::character varying))::text))) AS word
           FROM ((public.contentLists p
             JOIN public.users u ON ((p.contentList_owner_id = u.user_id)))
             JOIN public.aggregate_contentList a ON ((a.contentList_id = p.contentList_id)))
          ...
          GROUP BY p.contentList_id, p.contentList_name, p.contentList_owner_id, u.handle, u.name, a.repost_count) words
  WITH NO DATA;


CREATE UNIQUE INDEX IF NOT EXISTS contentList_row_number_idx ON public.contentList_lexeme_dict USING btree (row_number);
CREATE INDEX IF NOT EXISTS contentList_user_handle_idx ON public.contentList_lexeme_dict USING btree (handle);
CREATE INDEX IF NOT EXISTS contentList_user_name_idx ON public.contentList_lexeme_dict USING gin (user_name public.gin_trgm_ops);
CREATE INDEX IF NOT EXISTS contentList_words_idx ON public.contentList_lexeme_dict USING gin (word public.gin_trgm_ops);
