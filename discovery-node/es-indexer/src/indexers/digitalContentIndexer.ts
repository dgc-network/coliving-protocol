...
import { DigitalContentDoc } from '../types/docs'
...
export class DigitalContentIndexer extends BaseIndexer<DigitalContentDoc> {
  constructor() {
    super('digitalContents', 'digital_content_id')
    this.batchSize = 500
  }

  mapping: IndicesCreateRequest = {
    index: indexNames.digitalContents,
    ...
    mappings: {
      ...
      properties: {
        ...
        'remix_of.digitalContents.parent_digital_content_id': { type: 'keyword' },
      },
    },
  }
  ...
  baseSelect(): string {
    ...
    select 
      digitalContents.*,
      (digitalContents.download->>'is_downloadable')::boolean as downloadable,
      ...
      array(
        ...
        where
          r.digital_content_id = digitalContents.digital_content_id
        order by is_current
      ) as routes,
    ...
    WHERE digitalContents.is_current = true 
  ...
  checkpointSql(checkpoint: BlocknumberCheckpoint): string {
    ...
    and digital_content_id in (
      select digital_content_id from digitalContents where is_current and blocknumber >= ${checkpoint.digitalContents}
      union
