...
def test_search_digital_content_tags(app):
    """Tests that search by tags works fopr digitalContents"""
    ...
    with db.scoped_session() as session:
        ...
        digitalContents = search_digital_content_tags(session, args)

        assert len(digitalContents) == 3
        assert digitalContents[0]["digital_content_id"] == 5  # First w/ 3 plays
        assert digitalContents[1]["digital_content_id"] == 2  # Sec w/ 2 plays
        assert digitalContents[2]["digital_content_id"] == 4  # Third w/ 1 plays

        # DigitalContent id 6 does not appear b/c kpop and pop are not exact matches
