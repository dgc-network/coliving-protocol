...
def test_search_digital_content_tags(app):
    ...
    with db.scoped_session() as session:
        ...
        agreements = search_digital_content_tags(session, args)

        assert len(agreements) == 3
        assert agreements[0]["digital_content_id"] == 5  # First w/ 3 plays
        assert agreements[1]["digital_content_id"] == 2  # Sec w/ 2 plays
        assert agreements[2]["digital_content_id"] == 4  # Third w/ 1 plays
