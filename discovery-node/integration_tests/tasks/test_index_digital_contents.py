...
from src.tasks.digitalContents import (
...
cid_metadata_client = CIDMetadataClient(
    {
        multihash: {
            ...
            "remix_of": {"digitalContents": [{"parent_digital_content_id": 75808}]},
            "repost_count": 12,
            "save_count": 21,
            "description": None,
...
def test_index_digital_contents(mock_index_task, app):
    """Tests that digitalContents are indexed correctly"""
    ...
    with db.scoped_session() as session, challenge_event_bus.use_scoped_dispatch_queue():
        ...
        digital_content_routes = session.query(DigitalContentRoute).filter(DigitalContentRoute.digital_content_id == 1).all()
        ...
        digital_content_routes = session.query(DigitalContentRoute).filter(DigitalContentRoute.digital_content_id == 40).all()
        assert [
            route
            for route in digital_content_routes
        ...
        # Check that digital_content routes are assigned appropriately
        digital_content_routes = session.query(DigitalContentRoute).filter(DigitalContentRoute.digital_content_id == 30).all()
        assert [
            route
            for route in digital_content_routes
            if route.is_current is True
        ...
        # ================== Test Revert DigitalContentRoute ===================
        ...
        digital_content_route = (
            ...
            .filter(DigitalContentRoute.digital_content_id == 1, DigitalContentRoute.is_current == False)
            .all()
        )
        ...
        digital_content_routes = session.query(DigitalContentRoute).all()
...
def test_digital_content_indexing_skip_tx(app, mocker):
    """Tests that digitalContents skip cursed txs without throwing an error and are able to process other tx in block"""
    with app.app_context():
        db = get_db()
        challenge_event_bus: ChallengeEventBus = setup_challenge_bus()
        web3 = Web3()
    ...
    class TestDigitalContentTransaction:
        pass

    blessed_tx_hash = (
        "0x34004dfaf5bb7cf9998eaf387b877d72d198c6508608e309df3f89e57def4db3"
    )
    blessed_tx = TestDigitalContentTransaction()
    ...
    cursed_tx = TestDigitalContentTransaction()
    ...
    mocker.patch(
        "src.tasks.digitalContents.lookup_digital_content_record",
        side_effect=[cursed_digital_content_record, blessed_digital_content_record],
        autospec=True,
    )
    mocker.patch(
        "src.tasks.digitalContents.get_digital_content_events_tx",
        side_effect=[
