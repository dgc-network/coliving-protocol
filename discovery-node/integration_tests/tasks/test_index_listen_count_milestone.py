...
def get_next_digital_content_milestone(play_count: int, prev_milestone: Optional[int] = None):
    """
    ...
    if prev_milestone:
        if prev_milestone in next_threshold:
            ...
            # digital_content is past the last milestone, so return none and stop
            return None
...
def test_get_next_digital_content_milestone():
    next_milestone = get_next_digital_content_milestone(8)
    assert next_milestone == None

    next_milestone = get_next_digital_content_milestone(10)
    assert next_milestone == 10

    next_milestone = get_next_digital_content_milestone(520)
    assert next_milestone == 500

    # Test get next milestone with prev milestone defined
    next_milestone = get_next_digital_content_milestone(12, 10)
    assert next_milestone == None

    next_milestone = get_next_digital_content_milestone(28, 10)
    assert next_milestone == 25

    next_milestone = get_next_digital_content_milestone(50, 10)
    assert next_milestone == 50

    next_milestone = get_next_digital_content_milestone(520, 10)
    assert next_milestone == 500

    next_milestone = get_next_digital_content_milestone(1000100, 100000)
    assert next_milestone == 1000000

    next_milestone = get_next_digital_content_milestone(1000100, 1000000)
    assert next_milestone == None
