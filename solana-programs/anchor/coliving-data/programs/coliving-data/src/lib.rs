//! The Coliving Data Program is intended to bring all user data functionality to Solana through the
//! Anchor framework
pub mod constants;
pub mod error;
pub mod utils;
...
pub mod coliving_data {
    ...
    */
    ...
    use super::*;
    /// Initialize an instance of Coliving with admin keypair.
    ...
    /// Important to note that the metadata object is simply logged out to be picked up by the Coliving indexing layer.
    pub fn init_user(
        ...
        // Confirm that the base used for user account seed is derived from this Coliving admin storage account
    ...
    pub fn create_content_node(
        ...
        // Confirm that the base used for content node account seed is derived from this Coliving admin storage account
        let (derived_base, _) = Pubkey::find_program_address(
            &[&ctx.accounts.admin.key().to_bytes()[..32]],
            ctx.program_id,
        );
...
/// must be invoked from an existing Coliving `admin` account.
#[derive(Accounts)]
