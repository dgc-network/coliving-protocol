...
anchor build
COLIVING_DATA_PROGRAM_ID=$(solana-keygen pubkey target/deploy/audius_data-keypair.json)
sed -i "s/$cur_address/$COLIVING_DATA_PROGRAM_ID/g" Anchor.toml
sed -i "s/$cur_address/$COLIVING_DATA_PROGRAM_ID/g" programs/audius-data/src/lib.rs
