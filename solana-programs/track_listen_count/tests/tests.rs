...
async fn setup() -> (BanksClient, Keypair, Hash, Keypair, Keypair) {
    ...
    test_solana_env.add_program(
        ...
        processor!(coliving_eth_registry::processor::Processor::process),
    );
    ...
    create_account(
        ...
        &recent_blockhash,
        &signer_group,
        coliving_eth_registry::state::SignerGroup::LEN,
    )
    .await
    .unwrap();
...
async fn process_tx_init_signer_group(
    ...
    let mut transaction = Transaction::new_with_payer(
        &[
            coliving_eth_registry::instruction::init_signer_group(&coliving_eth_registry::id(), signer_group, group_owner)
                .unwrap(),
        ],
        Some(&payer.pubkey()),
    );
...
async fn process_tx_init_valid_signer(
    ...
    recent_blockhash: Hash,
    banks_client: &mut BanksClient,
    eth_address: [u8; coliving_eth_registry::state::SecpSignatureOffsets::ETH_ADDRESS_SIZE],
) -> Result<(), TransportError> {
    let mut transaction = Transaction::new_with_payer(
        &[coliving_eth_registry::instruction::init_valid_signer(
            &coliving_eth_registry::id(),
            valid_signer,
            signer_group,
            &group_owner.pubkey(),
...
fn construct_eth_address(
    pubkey: &PublicKey,
) -> [u8; coliving_eth_registry::state::SecpSignatureOffsets::ETH_ADDRESS_SIZE] {
    let mut addr = [0u8; coliving_eth_registry::state::SecpSignatureOffsets::ETH_ADDRESS_SIZE];
    addr.copy_from_slice(&sha3::Keccak256::digest(&pubkey.serialize()[1..])[12..]);
    assert_eq!(
        addr.len(),
        coliving_eth_registry::state::SecpSignatureOffsets::ETH_ADDRESS_SIZE
    );
    addr
}
...
async fn test_call_track_listen_instruction() {
    ...
    let start = 1;
    let end = start + coliving_eth_registry::state::SecpSignatureOffsets::SIGNATURE_OFFSETS_SERIALIZED_SIZE;

    let offsets = coliving_eth_registry::state::SecpSignatureOffsets::try_from_slice(
        &secp256_program_instruction.data[start..end],
    )
    .unwrap();

    let sig_start = offsets.signature_offset as usize;
    let sig_end = sig_start + coliving_eth_registry::state::SecpSignatureOffsets::SECP_SIGNATURE_SIZE;

    let mut signature: [u8; coliving_eth_registry::state::SecpSignatureOffsets::SECP_SIGNATURE_SIZE] =
        [0u8; coliving_eth_registry::state::SecpSignatureOffsets::SECP_SIGNATURE_SIZE];
    ...
    create_account(
        ...
        &recent_blockhash,
        &valid_signer,
        coliving_eth_registry::state::ValidSigner::LEN,
    )
    .await
    .unwrap();
