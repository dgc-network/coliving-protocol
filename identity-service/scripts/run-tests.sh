...
export solanaDigitalContentListenCountAddress=''
...
# tests, we use coliving_identity_service_test. The below block checks if
# coliving_identity_service_test exists, and if not, creates it (the tests will fail if this
...
# In CircleCI, the docker environment variables set up coliving_identity_service_test instead of
# coliving_identity_service.
...
if [ -z "${isCIBuild}" ]; then
  ...
  docker exec -i $DB_CONTAINER /bin/sh -c "psql -U postgres -tc \"SELECT 1 FROM pg_database WHERE datname = 'coliving_identity_service_test'\" | grep -q 1 || psql -U postgres -c \"CREATE DATABASE coliving_identity_service_test\""
elif [ -x "$(command -v psql)" ]; then
  psql -U postgres -h localhost -p $PG_PORT -tc "SELECT 1 FROM pg_database WHERE datname = 'coliving_identity_service_test'" | grep -q 1 || psql -U postgres -h localhost -p $PG_PORT -c "CREATE DATABASE coliving_identity_service_test"
fi
