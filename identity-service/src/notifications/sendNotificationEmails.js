...
async function processEmailNotifications (expressApp, colivingLibs) {
  try {
...
async function renderAndSendNotificationEmail (
  ...
  try {
    ...
    const emailSubject = `${notificationCount} unread notification${notificationCount > 1 ? 's' : ''} on Coliving`
