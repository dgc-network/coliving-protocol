...
class NotificationProcessor {
   ...
   * @param {Object} colivingLibs libs instance
   * @param {Object} expressApp express app context
   * @param {Object} redis redis connection
   */
  async init (colivingLibs, expressApp, redis) {
    ...
    this.notifQueue.process(async (job, done) => {
      ...
      try {
        ...
        if (minBlock < oldMaxBlockNumber) {
          ...
          maxBlockNumber = await this.indexAll(colivingLibs, optimizelyClient, minBlock, oldMaxBlockNumber)
        }
    ...
    this.solanaNotifQueue.process(async (job, done) => {
      ...
      try {
        ...
        if (minSlot < oldMaxSlot) {
          ...
          const optimizelyClient = expressApp.get('optimizelyClient')
          maxSlot = await this.indexAllSolanaNotifications(colivingLibs, optimizelyClient, minSlot, oldMaxSlot)
        }
    ...
    this.emailQueue.process(async (job, done) => {
      ...
      let error = null
      try {
        await processEmailNotifications(expressApp, colivingLibs)
    ...
    this.downloadEmailQueue.process(async (job, done) => {
      ...
      let error = null
      try {
        await processDownloadAppEmail(expressApp, colivingLibs)
   ...
   */
  async indexAll (colivingLibs, optimizelyClient, minBlock, oldMaxBlockNumber) {
    ...
    try {
      ...
      await sendNotifications(colivingLibs, notifications, tx, optimizelyClient)
      logger.info(`notifications main indexAll job - sendNotifications complete in ${Date.now() - time}ms`)
      time = Date.now()

      await indexMilestones(milestones, owners, metadata, listenCountWithOwners, colivingLibs, tx)
   ...
   */
  async indexAllSolanaNotifications (colivingLibs, optimizelyClient, minSlot, oldMaxSlot) {
    const startDate = Date.now()
    const startTime = process.hrtime()
    let time = startDate
    ...
    try {
      ...
      // Fetch additional metadata from DP, query for the user's notification settings, and send push notifications (mobile/browser)
      await sendNotifications(colivingLibs, processedNotifications, tx, optimizelyClient)
