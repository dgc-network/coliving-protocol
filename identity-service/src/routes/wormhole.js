...
/**
 * Transfers erc20 $DGC into spl $DGC via the wormhole
 ...
 * @param {*} colivingLibs Coliving Libs instance for completing the wormhole transfer
...
const relayWormhole = async (
  ...
  try {
    ...
    while ((!response || !response.transactionSignature) && numRetries < MAX_RETRIES) {
      ...
      response = await colivingLibs.wormholeClient.attestAndCompleteTransferEthToSol(transferTxHash, signTransaction, {
...
module.exports = function (app) {
  app.post('/wormhole_relay', async (req, res, next) => {
    const colivingLibs = req.app.get('colivingLibs')
    ...
    try {
      ...
      const {
        ...
        redeemTxSignature,
        error
      } = await relayWormhole(req, colivingLibs, body.senderAddress, body.permit, body.transferTokens, reportError)
      if (error) {
        return sendResponse(
          req,
          res,
