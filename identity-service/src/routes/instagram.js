  ...
  app.post('/instagram', handleResponse(async (req, res, next) => {
    ...
    try {
      ...
      if (existingInstagramUser) {
        return errorResponseBadRequest(`Another Coliving profile has already been authenticated with Instagram user @${igUser.username}!`)
      } else {
    ...
    try {
      ...
      if (instagramObj && !instagramObj.blockchainUserId) {
        ...
        if (instagramObj.verified) {
          const [encodedABI, contractAddress] = await colivingLibsInstance.User.updateIsVerified(
            userId, true, config.get('userVerifierPrivateKey')
          )
          const contractRegKey = await colivingLibsInstance.contracts.getRegistryContractForAddress(contractAddress)
