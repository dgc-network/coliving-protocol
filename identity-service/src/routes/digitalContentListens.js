...
const { createDigitalContentListenTransaction, getFeePayerKeypair, sendAndSignTransaction } = require('../solana-client')
...
const getDigitalContentListens = async (
  idList,
  timeFrame = undefined,
  startTime = undefined,
  endTime = undefined,
  ...
  if (idList && idList.length > 0) {
    dbQuery.where.digitalContentId = { [models.Sequelize.Op.in]: idList }
  }
  ...
  let listenCounts = await models.DigitalContentListenCount.findAll(dbQuery)
  let output = {}
  for (let i = 0; i < listenCounts.length; i++) {
    ...
    let digitalContentId = values.digitalContentId
    if (!output.hasOwnProperty(date)) {
      output[date] = {}
      output[date]['utcMilliseconds'] = values['date'].getTime()
      output[date]['totalListens'] = 0
      output[date]['digitalContentIds'] = []
      output[date]['listenCounts'] = []
    }

    output[date]['totalListens'] += listens
    if (!output[date]['digitalContentIds'].includes(digitalContentId)) {
      output[date]['digitalContentIds'].push(digitalContentId)
    }
...
const getTrendingDigitalContents = async (
  idList,
  timeFrame,
  limit,
  offset) => {
  ...
  let dbQuery = {
    attributes: ['digitalContentId', [models.Sequelize.fn('sum', models.Sequelize.col('listens')), 'listens']],
    group: ['digitalContentId'],
    order: [[models.Sequelize.col('listens'), 'DESC'], [models.Sequelize.col('digitalContentId'), 'DESC']],
    where: {}
  }
  ...
  let seenDigitalContentIds = []
  listenCounts.forEach((elem) => {
    parsedListenCounts.push({ digitalContentId: elem.digitalContentId, listens: parseInt(elem.listens) })
    seenDigitalContentIds.push(elem.digitalContentId)
  })
...
module.exports = function (app) {
  ...
  app.post('/digital_contents/:id/listen', handleResponse(async (req, res) => {
    ...
    const redis = req.app.get('redis')
    const digitalContentId = parseInt(req.params.id)
    const userId = req.body.userId
    if (!userId || !digitalContentId) {
    ...
    if (solanaListen) {
      ...
      try {
        ...
        req.logger.info(`DigitalContentListen location fetch failed: ${e}`)
        location = {}
      }

      try {
        let digitalContentListenTransaction = await createDigitalContentListenTransaction({
          validSigner: null,
          privateKey: config.get('solanaSignerPrivateKey'),
          userId: userId.toString(),
          digitalContentId: digitalContentId.toString(),
          source: 'relay',
          location,
          connection
        })
        ...
        if (sendRawTransaction) {
          req.logger.info(`DigitalContentListen tx submission, digitalContentId=${digitalContentId} userId=${userId} - sendRawTransaction`)
          solTxSignature = await sendAndSignTransaction(
            connection,
            digitalContentListenTransaction,
            feePayerAccount,
            timeout,
            logger
          )
          ...
          await retry(async () => {
            req.logger.info(`DigitalContentListen tx submission, digitalContentId=${digitalContentId} userId=${userId} - sendAndConfirmTransaction`)
            ...
            onRetry: (err, i) => {
              if (err) {
                req.logger.error(`DigitalContentListens tx retry error, digitalContentId=${digitalContentId} userId=${userId} : ${err}`)
              }
            }
          })
        }

        req.logger.info(`DigitalContentListen tx confirmed, ${solTxSignature} userId=${userId}, digitalContentId=${digitalContentId}, sendRawTransaction=${sendRawTransaction}`)
        ...
        return errorResponseServerError(`DigitalContentListens tx error, digitalContentId=${digitalContentId} userId=${userId} : ${e}`)
      }
    }

    // TODO: Make all of this conditional based on request parameters
    let digitalContentListenRecord = await models.DigitalContentListenCount.findOrCreate(
      {
        where: { hour: currentHour, digitalContentId }
      })
    if (digitalContentListenRecord && digitalContentListenRecord[1]) {
      logger.info(`New digital_content listen record inserted ${digitalContentListenRecord}`)
    }
    await models.DigitalContentListenCount.increment('listens', { where: { hour: currentHour, digitalContentId: req.params.id } })

    // Clients will send a randomly generated string UUID for anonymous users.
    // Those listened should NOT be recorded in the userDigitalContentListen table
    const isRealUser = typeof userId === 'number'
    if (isRealUser) {
      // Find / Create the record of the user listening to the digital_content
      const [userDigitalContentListenRecord, created] = await models.UserDigitalContentListen
        .findOrCreate({ where: { userId, digitalContentId } })

      // If the record was not created, updated the timestamp
      if (!created) {
        await userDigitalContentListenRecord.increment('count')
        await userDigitalContentListenRecord.save()
      }
    }
   ...
   *  digitalContents/history/
   *    - digitalContents w/ recorded listen event sorted by date listened
   *
   ...
   */
  app.get('/digital_contents/history', handleResponse(async (req, res) => {
    ...
    const digitalContentListens = await models.UserDigitalContentListen.findAll({
      where: { userId },
      order: [['updatedAt', 'DESC']],
      attributes: ['digitalContentId', 'updatedAt'],
      limit,
      offset
    })

    return successResponse({
      digitalContents: digitalContentListens.map(digital_content => ({ digitalContentId: digital_content.digitalContentId, listenDate: digital_content.updatedAt }))
    })
  }))

  /*
   * Return digital_content listen history grouped by a specific time frame
   *  digitalContents/listens/
   *    - all digitalContents, sorted by play count
   *
   *  digitalContents/listens/<time>
   ...
   */
  app.post('/digital_contents/listens/:timeframe*?', handleResponse(async (req, res, next) => {
    let body = req.body
    ...
    let output = await getDigitalContentListens(
      idList,
      time,
      startTime,
      endTime,
  ...
  app.get('/digital_contents/listens/:timeframe*?', handleResponse(async (req, res) => {
   ...
   *  digitalContents/trending/
   *    - all digitalContents, sorted by play count
   *
   *  digitalContents/trending/<time>
   *    - <time> - day, week, month, year
   *    - returns all digitalContents for given time period, sorted by play count
   *
   *  POST body parameters (optional):
   *    limit (int) - limits number of results
   *    offset (int) - offset results
   ...
   *    id (array of int) - filter results for specific digital_content(s)
   */
  app.post('/digital_contents/trending/:time*?', handleResponse(async (req, res) => {
    let time = req.params.time
    let body = req.body
    let idList = body.digital_content_ids
    let { limit, offset } = getPaginationVars(body.limit, body.offset)
    let parsedListenCounts = await getTrendingDigitalContents(
      idList,
      time,
      limit,
      offset)
  ...
  app.get('/digital_contents/trending/:time*?', handleResponse(async (req, res) => {
    let time = req.params.time
    let idList = req.query.id
    let { limit, offset } = getPaginationVars(req.query.limit, req.query.offset)
    let parsedListenCounts = await getTrendingDigitalContents(
  ...
  /*
   * Gets the digitalContents and listen counts for a user.
   ...
   *  limit: (optional) The number of digitalContents to fetch
   */
  app.get('/users/listens/top', authMiddleware, handleResponse(async (req, res) => {
    const { blockchainUserId: userId } = req.user
    const { limit = 25 } = req.query

    const listens = await models.UserDigitalContentListen.findAll({
      where: {
        userId: {
  ...
  /*
   * Gets whether or not digitalContents have been listened to by a target user.
   * Useful in filtering out digitalContents that a user has already listened to.
   * Requires auth.
   *
   * GET query parameters:
   *  digitalContentIdList: The ids of digitalContents to check
   */
  app.get('/users/listens', authMiddleware, handleResponse(async (req, res) => {
    const { blockchainUserId: userId } = req.user
    const { digitalContentIdList } = req.query

    if (!digitalContentIdList || !Array.isArray(digitalContentIdList)) {
    ...
    const listens = await models.UserDigitalContentListen.findAll({
      where: {
        ...
        digitalContentId: {
          [Sequelize.Op.in]: digitalContentIdList
        }
      }
    })

    const listenMap = listens.reduce((acc, listen) => {
      acc[listen.dataValues.digitalContentId] = listen.dataValues.count
      return acc
    }, {})

    digitalContentIdList.forEach(id => {
      if (!(id in listenMap)) {
        listenMap[id] = 0
      }
    })
  ...
  app.get('/listens/bulk', handleResponse(async (req, res) => {
    ...
    const userListens = await models.UserDigitalContentListen.findAll({
      attributes: { exclude: ['id'] },
      where: {
        updatedAt: { [models.Sequelize.Op.gt]: updatedAtMoment.toDate() }
      },
      order: [['updatedAt', 'ASC'], ['digitalContentId', 'ASC']],
      limit
    })

    const anonListens = await models.DigitalContentListenCount.findAll({
      attributes: ['digitalContentId', ['listens', 'count'], ['hour', 'createdAt'], 'updatedAt'],
      where: {
        updatedAt: { [models.Sequelize.Op.gt]: updatedAtMoment.toDate() }
      },
      order: [['updatedAt', 'ASC'], ['digitalContentId', 'ASC']],
