...
const sendBrowserNotification = async ({ userId, notificationParams }) => {
  ...
  try {
    ...
    await Promise.all(notificationBrowsers.map(async (notificationBrowser) => {
      ...
      try {
        await webpush.sendNotification(pushSubscription, JSON.stringify({ message, title: `${title} • Coliving` }))
        numSentNotifs++
      } catch (err) {
...
let apnAuthKeyPath = path.resolve(__dirname, './notifications/browserPush/coliving.pushpackage/AuthKey.p8')
if (apnAuthKey) {
  fs.writeFileSync(apnAuthKeyPath, apnAuthKey)
}
const apnTopic = config.get('environment') === 'staging'
  ? 'web.co.coliving.staging'
  : 'web.co.coliving'
...
const sendSafariNotification = async ({ userId, notificationParams }) => {
  ...
  try {
    ...
    note.alert = { title: `${title} • Coliving`, body: message }
    note.urlArgs = []
    note.payload = {}
