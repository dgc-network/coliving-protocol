...
describe('Test Favorite Notification', function () {
  ...
  it('should insert rows into notifications and notifications actions tables', async function () {
    ...
    // User 11 subscribes to user 1 and gets the notification when user 1 creates agreements 1, 2, 3, and contentList 1 which contains digital_content 2
    const user11Notifs = await models.Notification.findAll({ where: { userId: 11 } })
    assert.deepStrictEqual(user11Notifs.length, 2)
    const user11AgreementNotifs = user11Notifs.find(notif => notif.type === notificationTypes.Create.digital_content)
    ...
    const user10AgreementNotifs = user10Notifs.find(notif => notif.type === notificationTypes.Create.digital_content && notif.entityId === 2)
    ...
    assert.deepStrictEqual(user10AgreementNotifActions[0].actionEntityId, 4) // DigitalContent ID 4 was created
    ...
    // User 11 viewed their notifications, so there should be 1 new digital_content notification (agreementId 5 made by user 1)
    const user11newNotifs = await models.Notification.findAll({ where: { userId: 11 } })
    assert.deepStrictEqual(user11newNotifs.length, 3)
    const user11NewAgreementNotif = user11newNotifs.find(notif => notif.type === notificationTypes.Create.digital_content && notif.isViewed === false)
    ...
    const user12UpdatedAgreementNotif = user12UpdatedNotifs.find(notif => notif.type === notificationTypes.Create.digital_content)
    ...
    const user10AgreementNotifs2 = updatedUser10Notifs.find(notif => notif.type === notificationTypes.Create.digital_content && notif.entityId === 2)
