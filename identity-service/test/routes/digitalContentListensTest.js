...
describe('test Solana listen tracking', function () {
  ...
  beforeEach(async () => {
    delete require.cache[require.resolve('../../src/routes/digitalContentListens')]
    sandbox = sinon.createSandbox()
    createDigitalContentListenTransactionStub = sandbox.stub(solClient, 'createDigitalContentListenTransaction')
  ...
  const recordSuccessfulListen = async (raw) => {
    ...
    createDigitalContentListenTransactionStub.resolves('expected success')
  ...
  const recordFailedListen = async (raw) => {
    // Failed listen just needs createDigitalContentListenTransaction to fail because it's called before raw/non-raw logic
  ...
  const verifySuccessfulListens = async (numSuccess, numSubmission, percentSuccess, threshold) => {
    const resp = await request(app)
      .get(`/digital_contents/listen/solana/status?percent=${threshold}`)
      .send()
    assert.strictEqual(resp.status, 200)
  ...
  const verifyListensInRange = async (range, listens, listensInRange) => {
    const resp = await request(app)
      .get(`/digital_contents/listen/solana/status?cutoffMinutes=${range}`)
      .send()
    assert.strictEqual(resp.status, 200)
    assert.match(resp.headers['content-type'], /json/)
    const status = resp.body
