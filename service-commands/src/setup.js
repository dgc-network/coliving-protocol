...
// Path to coliving-protocol
const PROTOCOL_DIR = config.get('protocol_dir')

// `cd` command to coliving-protocol
...
/** Executes shell command in coliving protocol directory, and logs the stdout and stderr streams.
...
const discoveryNodeUp = async (options = { verbose: false }) => {
  ...
  // Start up the docker network `coliving_dev` and the Solana test validator
...
/**
 * Brings up an entire Coliving Protocol stack.
