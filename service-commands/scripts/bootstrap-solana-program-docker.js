...
const main = async () => {
  ...
  execSync(
    `cd ${solanaProgramsPath} && docker run --name coliving_solana_programs --network host -e SOLANA_HOST='http://127.0.0.1:8899' -v "$PWD:/usr/src/app" coliving/solana-programs:develop2 > solana-program-config.json`,
    { stdio: 'inherit' }
  )
  ...
  execSync(`cp ${solanaProgramsPath}/anchor/coliving-data/*.json ${tmp}`, {
    stdio: 'inherit'
  })

  execSync(`mkdir -p ${tmp}/coliving-data`, { stdio: 'inherit' })
  execSync(
    `cp -r ${solanaProgramsPath}/anchor/coliving-data/target ${tmp}/coliving-data`,
    { stdio: 'inherit' }
  )
  ...
  // Make the dockerfile
  execSync(
    `cd ${tmp} && docker build -f Dockerfile.cacheLedger -t coliving/solana-programs:predeployed-latest .`,
    { stdio: 'inherit' }
  )
  execSync(
    `docker tag coliving/solana-programs:predeployed-latest coliving/solana-programs:predeployed-${gitHash}`,
    { stdio: 'inherit' }
  )
  execSync(
    'docker kill coliving_solana_programs && docker rm coliving_solana_programs',
    { stdio: 'inherit' }
  )

  // TODO: Deploy to docker registry (dockerhub)
  console.log('Be sure to publish the docker file if using externally')
  console.log('ie. "docker push coliving/solana-programs:predeployed-latest"')
}
