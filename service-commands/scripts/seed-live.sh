...
seed-digitalcoin () {
    ...
    jq .ownerWallet < ~/.coliving/solana-program-config.json > ~/.coliving/owner-wallet.json
    solana config set -k ~/.coliving/owner-wallet.json
    echo "Minting Audio..."
...
# Transfer digitalcoin from user bank to user bank
tip-digitalcoin () {
    sender_id=$1
    receiver_id=$2
    amount=$3
