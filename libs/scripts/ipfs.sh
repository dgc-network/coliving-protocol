...
if [[ "$1" == 'up' ]]; then
  ...
  if [[ -z "$5" ]]; then
    docker run -d --name $CONTAINER_NAME -p 127.0.0.1:$API_PORT:5001 -p 127.0.0.1:$SWARM_PORT:4001 --network=coliving_dev $IPFS_RELEASE_VERSION daemon
  else
    GATEWAY_PORT=$5
    docker run -d --name $CONTAINER_NAME -p 127.0.0.1:$API_PORT:5001 -p 127.0.0.1:$SWARM_PORT:4001 -p 127.0.0.1:$GATEWAY_PORT:8080 --network=coliving_dev $IPFS_RELEASE_VERSION daemon
  fi
