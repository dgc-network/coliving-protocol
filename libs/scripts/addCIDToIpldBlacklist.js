...
async function run () {
  try {
    ...
    const colivingLibs = await createAndInitLibs(BLACKLISTER_PUBLIC_KEY)
    const ipldTxReceipt = await addIPLDTxToChain(colivingLibs, decodedCID.digest)
...
async function addIPLDTxToChain (colivingLibs, digest) {
  let ipldTxReceipt
  try {
    ipldTxReceipt = await colivingLibs.contracts.IPLDBlacklistFactoryClient.addIPLDToBlacklist(
      digest
    )
  } catch (e) {
