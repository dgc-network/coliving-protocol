...
.colivingLoginButton:hover {
  background: #D127E3;
  transform: perspective(1px) scale3d(1.04, 1.04, 1.04);
}

.colivingLoginButton.disableHoverGrow:hover {
  transform: none;
}

.colivingLoginButton:active {
  background: #A30CB3;
}

.colivingLoginButton.pill {
  border-radius: 99px;
}

.colivingLoginButton.fullWidth {
  width: 100%;
}

.colivingLoginButton.small {
  height: 20px;
  font-size: 11px;
  padding: 0px 32px;
}

.colivingLoginButton.large {
  height: 40px;
  font-size: 18px;
  padding: 0px 18px;
}
...
export class Oauth {
  ...
  renderButton(element: HTMLElement, options?: ButtonOptions) {
    if (!element) {
      console.error('Target element for Coliving OAuth button is empty.')
    }
    const style = document.createElement('style')
    style.textContent = CSS
    document.head.appendChild(style)
    const button = document.createElement('button')
    button.id = 'coliving-login-button'
    button.classList.add('colivingLoginButton')
